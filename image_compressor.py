...
class ImageCompressorApp:
    def __init__(self, root):
        ...
        # 输入路径
        tk.Label(root, text="输入路径:").grid(
            row=0, column=0, padx=10, pady=5, sticky="w"
        )
        self.input_entry = tk.Entry(root)
        self.input_entry.grid(row=0, column=1, padx=10, pady=5, sticky="ew")
        self.input_entry.insert(0, os.path.join(".", "input"))
        tk.Button(root, text="选择文件", command=self.select_file, width=10).grid(
            row=0, column=2, padx=2, pady=5
        )
        tk.Button(root, text="选择文件夹", command=self.select_folder, width=12).grid(
            row=0, column=3, padx=2, pady=5
        )

        # 输出路径
        tk.Label(root, text="输出路径:").grid(
        ...
        tk.Button(
            root, text="选择文件夹", command=self.select_output_folder, width=12
        ).grid(row=1, column=2, columnspan=2, padx=2, pady=5)

        # 目标大小
        tk.Label(root, text="目标大小 (KB):").grid(
        ...
        # Resize 功能
        tk.Label(root, text="Resize:").grid(
